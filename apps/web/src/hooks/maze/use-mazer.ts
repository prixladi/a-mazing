...
export const useMazer = (mazeConfig: MazeConfig, mazeMutations: MazeMutations) => {
  ...
  useEffect(() => {
    ...
    const entrypoints = mazeConfig.entrypoints.map(([x, y]) => MazerPosition.new(x, y));
    const checkpoints = mazeConfig.checkpoints.map(({ position: [x, y], level }) => {
  ...
  useEffect(() => {
    ...
    const softWalls = mazeMutations.softWalls.map(([x, y]) => MazerPosition.new(x, y));
    const result = mazer.run(softWalls);

    setScore(result?.score ?? null);
    setPath(result?.path?.map((pos) => [pos.x, pos.y]) ?? null);
  }, [mazeMutations.softWalls, mazer]);
